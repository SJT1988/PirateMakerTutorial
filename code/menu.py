...
class Menu:
    ...
    def create_buttons(self):
        ...
        self.coin_button_rect = generic_button_rect.move(self.rect.width / 2, 0) \
            .inflate(-button_margin, -button_margin)
        self.enemy_button_rect = generic_button_rect.move(0, self.rect.height / 2) \
            .inflate(-button_margin, -button_margin)
        self.palm_button_rect = generic_button_rect.move(self.rect.width / 2, self.rect.height / 2) \
            .inflate(-button_margin, -button_margin)
    ...
    def click(self, mouse_pos, mouse_button):
        for sprite in self.buttons:
            if sprite.rect.collidepoint(mouse_pos):
                if mouse_button[1] and sprite.items['alt']: # middle mouse click
                        sprite.main_active = not sprite.main_active
                if mouse_button[2]: # right mouse click
                    sprite.switch()
                return sprite.get_id()

    def highlight_indicator(self, index):
        if index not in range(2,19):
            return
...
class Button(pygame.sprite.Sprite):
    ...
    def switch(self):
        if self.index >= len(self.items['main' if self.main_active else 'alt']) - 1:
            self.index = 0
        else:
            self.index += 1
