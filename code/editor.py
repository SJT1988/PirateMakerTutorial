...
class Editor:
	def __init__(self, land_tiles):
		...
		# grid lines
		self.grid_line_surf = pygame.Surface((WINDOW_WIDTH, WINDOW_HEIGHT))
		self.grid_line_surf.set_colorkey('green')
		self.grid_line_surf.set_alpha(30)
		...
		self.object_timer = Timer(400) # milliseconds
		...
		self.sky_handle = CanvasObject(
			pos = vector((WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2)),
	...
	def check_neighbors(self, cell_pos):
		...
		if cluster_size < 3 or cluster_size % 2 == 0:
			raise ValueError("cluster_size must be 3")

		local_cluster = [
			(col, row)
			for col in range(cell_pos[0] - cluster_size // 2,
			cell_pos[0] + cluster_size // 2 + 1)
			for row in range(cell_pos[1] - cluster_size // 2,
			cell_pos[1] + cluster_size // 2 + 1)]
			...
			if cell in self.canvas_data:
				...
				for name, side in NEIGHBOR_DIRECTIONS.items():
					...
					if neighbor_cell in self.canvas_data:
						# water top neighbor
						if self.canvas_data[neighbor_cell].has_water \
	...
	def selection_hotkeys(self, event):
		if event.type == pygame.KEYDOWN:
			if (event.key == pygame.K_RIGHT):
				self.selection_index = min(18, self.selection_index + 1)
			if (event.key == pygame.K_LEFT):
				self.selection_index = max(2, self.selection_index - 1)
	...
	def draw_grid_lines(self):
		...
		self.grid_line_surf.fill('green')

		for col in range(cols + 1):
			x = origin_offset.x + col * TILE_SIZE
			pygame.draw.line(self.grid_line_surf, LINE_COLOR, (x, 0), (x, WINDOW_HEIGHT))

		for row in range(rows + 1):
			y = origin_offset.y + row * TILE_SIZE
			pygame.draw.line(self.grid_line_surf, LINE_COLOR, (0, y), (WINDOW_WIDTH, y))

		self.display_surface.blit(self.grid_line_surf,(0,0))
	
	def draw_sprite(self, sprite_idx, pos, cntr = False, mdbttm = False):
		frames = self.animations[sprite_idx]['frames']
		index = int(self.animations[sprite_idx]['frame index'])
		surf = frames[index]
		if not cntr and not mdbttm:
	...
	def draw_level(self):
		for cell_pos, tile in self.canvas_data.items():
			...
			# coins
			if tile.coin:
				self.draw_sprite(tile.coin, pos, True)

			# enemies
			if tile.enemy:
				self.draw_sprite(tile.enemy, pos, False, True)
...
class CanvasTile:
	...
	def check_content(self):
		if not self.has_terrain and \
		not self.has_water and \
		not self.coin and \
		not self.enemy:
			self.is_empty = True
...
class CanvasObject(pygame.sprite.Sprite):
	'''pos: object position
	frames: list of frames (surfaces) in the object
	tile_id: the int index of the object (see EDITOR DATA in settings.py)
	origin: origin of the editor (for some reason...see if this always happens. Then it doesn't need to be a param.)'''
